...
const SynthAudioModule = {
    ...
    triggerPurgeSound() {
        ...
        osc.frequency.setValueAtTime(350, this.audioCtx.currentTime + 0.06);
    ...
    triggerEngraveSound() {
        ...
        [0, 0.05, 0.1].forEach((delay, index) => {
            ...
            let gain = this.audioCtx.createGain();
            osc.type = 'sawtooth';
            osc.frequency.setValueAtTime(500 + (index * 250), time + delay);
...
// Clean Vector SVG URL Encodings mapping safely straight to CSS Background Masks 
const MASK_GLYPHS = {
    ...
    acoustic: `url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24'><path d='M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-4z'/></svg>")`,
...
// Architecture Blueprints (Completely open, paths and prerequisites removed)
const INITIAL_MAPS = {
    crucible: [
    ...
    wildcard: [
        ...
        { id: "wc_acoustic", name: "ACOUSTIC LANDSCAPES", mask: MASK_GLYPHS.acoustic, rank: 0, maxRank: 5, desc: "Deconstruct frequency manipulation and composition principles to construct rich acoustic tapestries." },
        { id: "wc_poly", name: "TOTAL POLYMATHY SYNTHESIS", mask: MASK_GLYPHS.poly, rank: 0, maxRank: 5, desc: "Capstone: Fuse biological longevity, somatic performance, sound logic, and system theory into an ultimate life practice." }
    ]
};
...
window.onload = () => {
    // Lock down initial drop down customizer state configurations
